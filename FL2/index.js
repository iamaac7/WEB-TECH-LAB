...
function validateForm(theForm) {
   with(theForm) {
      ...
      return (isNotEmpty(txtName, "Please enter your name!", elmNameError)
           ...
           && isValidPassword(txtPassword, "Password shall be Minimum 8 Characters long and must carry 1 Upper case, 1 Number and special character!", elmPasswordError)
...
// Validate password, minimum 8 characters of [a-zA-Z0-9_]
function isValidPassword(inputElm, errMsg, errElm) {
	// !regularExpression.test(newPassword)
	//var isValid = (inputElm.value.trim().test(/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,16}$/) != null);
   var isValid = (inputElm.value.trim().match(/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,16}$/) !== null);
   postValidate(isValid, errMsg, errElm, inputElm);
   return isValid;
}
